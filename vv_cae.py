...
def train():
    ...
    encoder, decoder, enc_args, dec_args = mlp_architecture_ala_iclr_18(n_pc_points, bneck_size,mode='fc')
    ...
    #loss_ri=tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square(fi-fr),axis=-1)))
    #loss_pi=tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square(fp-fi),axis=-1)))
    #loss_pr=tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square(fp-fr),axis=-1)))
    loss_luc=(mfi[0]+mfr[0]+mfp[0])/3

    #loss_rii=tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square(fii-frr),axis=-1)))

    #loss_mri=tf.reduce_mean(tf.reduce_sum(tf.square(mfi-mfr),axis=-1))
    #loss_mpi=tf.reduce_mean(tf.reduce_sum(tf.square(mfi-mfp),axis=-1))
   # loss_mpr=tf.reduce_mean(tf.reduce_sum(tf.square(mfp-mfr),axis=-1))
    ...
    loss_d=-tf.log(loss_ri+1e-5)+0.5*loss_pi#+loss_luc#(loss_pi+loss_pr)#-0.1*tf.log(loss_mri+1e-5)+0.1*loss_mpi
    ...
    loss_e=chamfer_big(pointcloud_pl,out)
    ...
    loss_d_local=loss_d+0.01*lde_zhengze#+0.01*tf.reduce_mean(tf.square(words))
    ...
    trainstep.append(tf.train.AdamOptimizer(learning_rate=0.003).minimize(loss_d_local, global_step=global_step,var_list=varf))#0.005
        ...
        for i in range(EPOCH_ITER_TIME):
            ...
            for j in range(FILE_NUM):
                ...
                for batch in range(batch_num):
                    ...
                    posis=jitter_point_cloud(posis,0.01,0.2)
                    ...
                    if (batch+1) % 16 == 0:
                        #meanval=0
                        #meanval2=mean(glist2)
                        #meanval3=mean(glist3)
                        #lastval=mean(glist)
                        #lastlocal=mean(glist2)
                        ...
                        #print('words: ',np.max(fpv,axis=-1)[0],np.max(frv,axis=-1)[0],np.max(fiv,axis=-1)[0])
                        #print('words: ',fpv[0],frv[0],fiv[0])
                        print('loss: ',resi[-1])
            #print('mean time', mean(tlist))
