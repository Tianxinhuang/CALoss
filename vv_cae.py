...
def train(args):    
    ...
    posi_pl=tf.placeholder(tf.float32,[args.batch_size,None,3],name='posi_pl')
    ...
    #calculate the loss with CALoss
    ...
    allgevars=[v for v in allvars if 'ge' in v.name]
    varad=[v for v in trainvars if '2ad' in v.name]
    bnvar=[v for v in allgevars if 'bnorm' in v.name]
    ...
    loss_d_local=loss_d_local+args.ca_reg*lde_zhengze

    #set trainstep and visualized losses
    trainstep=[]
    trainstep.append(tf.train.AdamOptimizer(learning_rate=args.ae_lr).minimize(loss_e, global_step=global_step,var_list=varge+bnvar))
    trainstep.append(tf.train.AdamOptimizer(learning_rate=args.ca_lr).minimize(loss_d_local, global_step=global_step,var_list=varad))
        ...
        for i in range(args.itertime):
            for j in range(filenum):
                ...
                for batch in range(batch_num):
                    ...
                    batch_point=copy.deepcopy(batch_point)
...
if __name__=='__main__':
    ...
    parser.add_argument('--savepath', type=str, default='./modelvv_cae/', help='The path of saved checkpoint')
    ...
    parser.add_argument('--wei_pi', type=float, default=0.003, help='The regularization parameter of reconstruction network')
    parser.add_argument('--wei_luc', type=float, default=2.0, help='The regularization parameter of CALoss')
    parser.add_argument('--perturb', type=float, default=0.003, help='The learning rate of reconstruction network')

    parser.add_argument('--ae_reg', type=float, default=0.001, help='The regularization parameter of reconstruction network')
    parser.add_argument('--ca_reg', type=float, default=0.0001, help='The regularization parameter of CALoss')
    parser.add_argument('--ae_lr', type=float, default=0.0001, help='The learning rate of reconstruction network')
    parser.add_argument('--ca_lr', type=float, default=0.003, help='The learning rate of CALoss')
